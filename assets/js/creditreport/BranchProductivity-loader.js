// =========================
// COMPONENT LOADER
// Same pattern as RepDetailbyBranch-loader.js.
// =========================
let loaded = 0;
async function loadComponent(id, file) {
    try {
        ...
        if (id === "topbar-container") {
            ...
            // Unlike Officer Productivity's back button (hardcoded back to
            // RepDetailbyCO.html), this one deliberately keeps the shared
            // topbar.js default (history.back()) instead of overriding it:
            // RepDetailbyBranch.js persists its full filter state (section/
            // view/dates) into its own URL via history.replaceState, so
            // history.back() from here returns to that exact same report
            // — a hardcoded plain URL would silently reset it to defaults,
            // undoing that.
        }
    } catch (error) {
        console.error(error);
